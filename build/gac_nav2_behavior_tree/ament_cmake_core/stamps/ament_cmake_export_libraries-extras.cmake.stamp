...
set(_exported_libraries "gac_nav2_behavior_tree;nav2_compute_path_to_pose_action_bt_node;nav2_compute_path_through_poses_action_bt_node;nav2_controller_cancel_bt_node;nav2_wait_cancel_bt_node;nav2_spin_cancel_bt_node;nav2_back_up_cancel_bt_node;nav2_assisted_teleop_cancel_bt_node;nav2_drive_on_heading_cancel_bt_node;nav2_smooth_path_action_bt_node;nav2_follow_path_action_bt_node;nav2_back_up_action_bt_node;nav2_drive_on_heading_bt_node;nav2_spin_action_bt_node;nav2_wait_action_bt_node;nav2_assisted_teleop_action_bt_node;nav2_clear_costmap_service_bt_node;nav2_is_stuck_condition_bt_node;nav2_transform_available_condition_bt_node;nav2_goal_reached_condition_bt_node;nav2_globally_updated_goal_condition_bt_node;nav2_goal_updated_condition_bt_node;nav2_is_path_valid_condition_bt_node;nav2_time_expired_condition_bt_node;nav2_path_expiring_timer_condition;nav2_distance_traveled_condition_bt_node;nav2_initial_pose_received_condition_bt_node;nav2_is_battery_charging_condition_bt_node;nav2_is_battery_low_condition_bt_node;nav2_reinitialize_global_localization_service_bt_node;nav2_rate_controller_bt_node;nav2_distance_controller_bt_node;nav2_speed_controller_bt_node;nav2_truncate_path_action_bt_node;nav2_truncate_path_local_action_bt_node;nav2_goal_updater_node_bt_node;nav2_path_longer_on_approach_bt_node;nav2_recovery_node_bt_node;nav2_navigate_to_pose_action_bt_node;nav2_navigate_through_poses_action_bt_node;nav2_remove_passed_goals_action_bt_node;nav2_pipeline_sequence_bt_node;nav2_round_robin_node_bt_node;nav2_single_trigger_bt_node;nav2_planner_selector_bt_node;nav2_controller_selector_bt_node;nav2_smoother_selector_bt_node;nav2_goal_checker_selector_bt_node;nav2_progress_checker_selector_bt_node;nav2_goal_updated_controller_bt_node")
set(_exported_library_names "")

# populate gac_nav2_behavior_tree_LIBRARIES
if(NOT _exported_libraries STREQUAL "")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^-" AND NOT "${_arg}" MATCHES "^-[l|framework]")
      list(APPEND gac_nav2_behavior_tree_LIBRARIES "${_arg}")
      math(EXPR _i "${_i} + 1")
      continue()
    endif()
    ...
    if("${_arg}" MATCHES "^(debug|optimized|general)$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'gac_nav2_behavior_tree' passes the build configuration keyword '${_cfg}' as the last exported library")
      endif()
    ...
    if(NOT IS_ABSOLUTE "${_library}")
      ...
      find_library(
        _lib NAMES "${_library}"
        PATHS "${gac_nav2_behavior_tree_DIR}/../../../lib"
        NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH
      )

      if(NOT _lib)
        # warn about not existing library and ignore it
        message(FATAL_ERROR "Package 'gac_nav2_behavior_tree' exports the library '${_library}' which couldn't be found")
      elseif(NOT IS_ABSOLUTE "${_lib}")
        # the found library must be an absolute path
        message(FATAL_ERROR "Package 'gac_nav2_behavior_tree' found the library '${_library}' at '${_lib}' which is not an absolute path")
      elseif(NOT EXISTS "${_lib}")
        # the found library must exist
        message(FATAL_ERROR "Package 'gac_nav2_behavior_tree' found the library '${_lib}' which doesn't exist")
      else()
        list(APPEND gac_nav2_behavior_tree_LIBRARIES ${_cfg} "${_lib}")
      endif()

    else()
      if(NOT EXISTS "${_library}")
        # the found library must exist
        message(WARNING "Package 'gac_nav2_behavior_tree' exports the library '${_library}' which doesn't exist")
      else()
        list(APPEND gac_nav2_behavior_tree_LIBRARIES ${_cfg} "${_library}")
      endif()
    endif()
  endwhile()
endif()
...
# and add the libraries to gac_nav2_behavior_tree_LIBRARIES
if(NOT _exported_library_names STREQUAL "")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^(debug|optimized|general)$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'gac_nav2_behavior_tree' passes the build configuration keyword '${_cfg}' as the last exported target")
      endif()
    ...
    if(NOT _library_dirs)
      ...
      if(NOT _lib)
        ...
        message(WARNING "Package 'gac_nav2_behavior_tree' exports library '${_library_name}' which couldn't be found")
      endif()
    else()
      ...
      if(NOT _lib)
        # warn about not existing library and later ignore it
        message(WARNING
          "Package 'gac_nav2_behavior_tree' exports library '${_library_name}' with LIBRARY_DIRS '${_library_dirs}' which couldn't be found")
      endif()
    endif()
    if(_lib)
      list(APPEND gac_nav2_behavior_tree_LIBRARIES ${_cfg} "${_lib}")
...
# TODO(dirk-thomas) deduplicate gac_nav2_behavior_tree_LIBRARIES
